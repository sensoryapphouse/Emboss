// Regression guard for the BANA Braille Formats 2016 §7 (Boxed Material) gold examples
// (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process
// against every gold file in tests/gold/bana-formats-2016/section-7/*.json and
// checks the result against tests/gold/bana-formats-2016/section-7/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-7 --update-status`,
//    never hand-edited).
//
...
// (optionally --update-status) whenever Emboss's box/table formatting changes,
// and update standards-findings.md/standards-map.md for anything that newly
// matches or newly breaks. See tests/gold/bana-formats-2016/section-7/README.md
// and compare-gold-7/differences.md (this reconciliation's own working notes)
// for how the gold files were built and what the section's known, documented
// mismatches are (standards-findings.md F-70, F-72, F-75, F-76, F-77 — box
// lines never narrow for line-numbered text, no colour/screened-box support,
// a box's own title is always a fixed cell-5 plain-text heading, this
// section's own print.blocks schema has no line-number representation, and
// formatTable's blanket inter-row blank line on any wrapped row is
// contradicted by this section's own Sample 7-1).
//
...
for (const { sample } of samples) {
  test(`gold §7 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-7 --sample ${sample.id} --update-status`).
