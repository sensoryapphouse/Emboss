// Regression guard for the BANA Braille Formats 2016 §3 (Transcriber's Notes) gold examples
// (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process against
// every gold file in tests/gold/bana-formats-2016/section-3/*.json and checks the result
// against tests/gold/bana-formats-2016/section-3/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-3 --update-status`,
//    never hand-edited).
//
...
// whenever Emboss's transcriber's-note/heading/paragraph/list formatting changes, and update
// standards-findings.md/standards-map.md for anything that newly matches or newly breaks.
// See tests/gold/bana-formats-2016/section-3/README.md and this reconciliation's own
...
// mismatches are (standards-findings.md F-15, F-107, F-138, F-163; the book's own "444"
// ellipsis truncation in example-3-4/sample-3-01 has no print-side input to build from — an
...
for (const { sample } of samples) {
  test(`gold §3 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-3 --sample ${sample.id} --update-status`).
