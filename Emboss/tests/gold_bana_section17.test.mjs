// Regression guard for the BANA Braille Formats 2016 §17 (Spelling Lists and Activities) gold
// examples (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process
// against every gold file in tests/gold/bana-formats-2016/section-17/*.json and checks the
// result against tests/gold/bana-formats-2016/section-17/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-17 --update-status`,
//    never hand-edited).
//
...
// whenever Emboss's word-list/glossary handling changes, and update standards-findings.md/
// standards-map.md for anything that newly matches or newly breaks. See tests/gold/
// bana-formats-2016/section-17/README.md and this reconciliation's own differences.md (kept
...
// (standards-findings.md F-183, F-185, F-211, F-212, F-213, F-235, F-236).
//
...
for (const { sample } of samples) {
  test(`gold §17 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-17 --sample ${sample.id} --update-status`).
