// Regression guard for the BANA Braille Formats 2016 §21 (Alphabetic References) gold examples
// (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process against every
// gold file in tests/gold/bana-formats-2016/section-21/*.json and checks the result against
// tests/gold/bana-formats-2016/section-21/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-21 --update-status`,
//    never hand-edited).
//
...
// bana-formats-2016/section-21/README.md and this reconciliation's own differences.md (kept
...
// (standards-findings.md F-191..F-204).
//
...
for (const { sample } of samples) {
  test(`gold §21 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-21 --sample ${sample.id} --update-status`).
