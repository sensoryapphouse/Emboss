// Regression guard for the BANA Braille Formats 2016 §18 (Grammar) gold examples
// (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process against every
// gold file in tests/gold/bana-formats-2016/section-18/*.json and checks the result against
// tests/gold/bana-formats-2016/section-18/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-18 --update-status`,
//    never hand-edited).
//
...
// whenever Emboss's grammar/diagram/transcriber's-note handling changes, and update
// standards-findings.md/standards-map.md for anything that newly matches or newly breaks. See
// tests/gold/bana-formats-2016/section-18/README.md and this reconciliation's own
...
// mismatches are (standards-findings.md F-186..F-190).
//
...
for (const { sample } of samples) {
  test(`gold §18 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-18 --sample ${sample.id} --update-status`).
