// Regression guard for the BANA Braille Formats 2016 §16 (Notes) AND UKAAF
// B004 §11 (Notes and footnotes) gold examples (standards-testing.md, stage
// 4-5). Runs Emboss/scripts/gold-run.mjs in-process against every gold file in
// both tests/gold/bana-formats-2016/section-16/*.json and
// tests/gold/ukaaf-b004/section-11/*.json, and checks each result against
// that section's own status.json:
//
...
//    never hand-edited).
//
// This file does not itself decide what "should" match — each section's own
// status.json is the current recorded truth, produced by the runner. Re-run
// gold-run.mjs (optionally --update-status) whenever Emboss's note/footnote/
// endnote formatting changes, and update standards-findings.md/
// standards-map.md for anything that newly matches or newly breaks. See
// tests/gold/bana-formats-2016/section-16/README.md and
// tests/gold/ukaaf-b004/section-11/README.md (how the gold files were built,
// and what each section's known, documented mismatches/limitations are —
// F-N1 through F-N15 in standards-findings.md) and this reconciliation's own
// differences.md (kept in the session's scratch folder per the task
// instructions, not in the repository).
//
...
// module is evaluated. Because gold-run.mjs is a single module whose SECTION
// (and derived GOLD_DIR/OPTS) are fixed at import time, this file dynamically
// imports it TWICE under two different `import(...)` specifiers (a `?section=`
// query suffix, ignored by Node's resolution but sufficient to force a fresh
// module instance per section) so the two sections' own GOLD_RUN_SECTION
// values don't collide within one process — matching gold_bana_section1.test.mjs's
// own precedent exactly (BANA §1 + UKAAF B004 §6-9).
...
const SECTIONS = [
  { id: 'section-16', label: 'BANA §16' },
  { id: 'b004-11', label: 'UKAAF B004 §11' },
];
