// Regression guard for the BANA Braille Formats 2016 §4 (Headings) gold examples
// (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process
// against every gold file in tests/gold/bana-formats-2016/section-4/*.json and
// checks the result against tests/gold/bana-formats-2016/section-4/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-4 --update-status`,
//    never hand-edited).
//
...
// (optionally --update-status) whenever Emboss's heading/paragraph/list
...
// bana-formats-2016/section-4/README.md and differences.md (this
...
// F-39, F-40, F-41; a handful of others are genuinely not representable from
// this section's flat print.blocks schema — see each file's own `uncertain`).
//
...
for (const { sample } of samples) {
  test(`gold §4 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-4 --sample ${sample.id} --update-status`).
