// Regression guard for the BANA Braille Formats 2016 §10 (Exercise Material) gold examples
// (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process against every
// gold file in tests/gold/bana-formats-2016/section-10/*.json and checks the result against
// tests/gold/bana-formats-2016/section-10/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-10 --update-status`,
//    never hand-edited).
//
...
// whenever Emboss's exercise-material handling changes, and update standards-findings.md/
// standards-map.md for anything that newly matches or newly breaks. See tests/gold/
// bana-formats-2016/section-10/README.md and this reconciliation's own differences.md (kept
...
// (standards-findings.md F-173..F-176, F-225..F-229, F-237).
//
...
for (const { sample } of samples) {
  test(`gold §10 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-10 --sample ${sample.id} --update-status`).
