// Regression guard for the BANA Braille Formats 2016 §1 (Basic Principles and
// General Formats) AND UKAAF B004 §6-9 gold examples (standards-testing.md,
// stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process against every gold
// file in both tests/gold/bana-formats-2016/section-1/*.json and
// tests/gold/ukaaf-b004/sections-6-9/*.json, and checks each result against
...
// gold-run.mjs (optionally --update-status) whenever Emboss's paragraph/list/
// heading/page-number/running-head formatting changes, and update
// standards-findings.md/standards-map.md for anything that newly matches or
// newly breaks. See tests/gold/bana-formats-2016/section-1/README.md and
// tests/gold/ukaaf-b004/sections-6-9/README.md (how the gold files were built,
// and what each section's known, documented mismatches/limitations are) and
// this reconciliation's own differences.md (kept in the session's scratch
// folder per the task instructions, not in the repository).
//
...
// values don't collide within one process.
...
const SECTIONS = [
  { id: 'section-1', label: 'BANA §1' },
  { id: 'b004-6-9', label: 'UKAAF B004 §6-9' },
];
