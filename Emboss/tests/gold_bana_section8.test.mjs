// Regression guard for the BANA Braille Formats 2016 §8 (Lists) gold examples
// (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process
// against every gold file in tests/gold/bana-formats-2016/section-8/*.json and
// checks the result against tests/gold/bana-formats-2016/section-8/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-8 --update-status`,
//    never hand-edited).
//
...
// (optionally --update-status) whenever Emboss's list/bullet/outline/box
...
// bana-formats-2016/section-8/README.md and differences.md (this
...
// F-33 through F-38 — no bullet-shape fidelity, no column layout, no
// list-aware pagination, no outline blank/guide-text machinery, no
// "directions" concept, emphasis not omitted for a uniformly-emphasised list —
// plus a handful of items genuinely not representable from this section's
// print.blocks schema; see each file's own `uncertain`/`resolution`).
//
...
for (const { sample } of samples) {
  test(`gold §8 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-8 --sample ${sample.id} --update-status`).
