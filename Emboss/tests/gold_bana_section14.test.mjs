// Regression guard for the BANA Braille Formats 2016 §14 (Plays, Cartoons, and Graphic Novels)
// gold examples (standards-testing.md, stage 4-5). Runs Emboss/scripts/gold-run.mjs in-process
// against every gold file in tests/gold/bana-formats-2016/section-14/*.json and checks the
// result against tests/gold/bana-formats-2016/section-14/status.json:
//
...
//    `node Emboss/scripts/gold-run.mjs --section section-14 --update-status`,
//    never hand-edited).
//
...
// whenever Emboss's play/cartoon/graphic-novel handling changes, and update
// standards-findings.md/standards-map.md for anything that newly matches or newly breaks. See
// tests/gold/bana-formats-2016/section-14/README.md for how the gold files were built and what
// the section's known, documented mismatches are (standards-findings.md F-206 (reused),
// F-240, F-241, F-242, and the two unresolved translator-table observations F-243/F-244).
//
...
for (const { sample } of samples) {
  test(`gold §14 ${sample.id}: matches iff status.json says it should`, () => {
    ...
    if (recorded.status === 'match') {
      ...
      // it (`node Emboss/scripts/gold-run.mjs --section section-14 --sample ${sample.id} --update-status`).
